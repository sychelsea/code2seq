...
def save_dictionaries(dataset_name, subtoken_to_count, node_to_count,
                      target_to_count, max_contexts, num_examples):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if len(contexts) > max_contexts_to_sample:
                    contexts = np.random.choice(contexts,
...
if __name__ == '__main__':
    ...
    if args.train_data_path == '' and args.val_data_path == '':
        num_examples = process_file(file_path=args.test_data_path,
                                    data_file_role='test',
                                    dataset_name=args.output_name,
                                    max_contexts=int(args.max_contexts),
                                    max_data_contexts=int(
                                        args.max_data_contexts))
        exit()
    ...
    for data_file_path, data_role in zip(
        [test_data_path, val_data_path, train_data_path],
        ['test', 'val', 'train']):
        num_examples = process_file(file_path=data_file_path,
